...
function renderBlockedHTML(payload: {
  ...
  const attackType = escapeHtml(String(payload.attack_type || 'unknown'))
...
<html lang="zh-CN">
  ...
  <body>
    <main class="shell">
      <span class="badge">Security policy blocked</span>
      <h1>403 - 攻击请求已被拦截</h1>
      <p>当前访问命中了校园供应链安全策略，相关证据已经进入安全事件中心，可继续查看来源 IP、攻击类型和处置结果。</p>
      <div class="grid">
        <div class="item">
...
function campusIdsRouteGuard() {
  return {
    name: 'campus-ids-route-guard',
        ...
        try {
          ...
          const response = await fetch(`${probeBase}/ids/browser-route-probe`, {
            ...
            headers: {
              ...
              'X-IDS-Browser-Probe': 'campus-vite-route-guard',
            },
            body: JSON.stringify({
              method,
              path: url.pathname,
...
export default defineConfig({
  plugins: [vue(), campusIdsRouteGuard()],
  resolve: {
    alias: {
